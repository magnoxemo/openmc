...
namespace openmc {
    ...
    namespace {
        ...
        {
            switch (a) {
                case LegendreAxis::x: return "x";
                case LegendreAxis::y: return "y";
                default:              return "z";
            }
        }
        ...
        {
            switch (a) {
                case LegendreAxis::x: return p.r().x;
                case LegendreAxis::y: return p.r().y;
                default:              return p.r().z;
            }
        }
    ...
    {
        bool has_axis_children = node.child("x") || node.child("y") || node.child("z");
        ...
        if (has_axis_children) {
            ...
            for (int d = 0; d < 3; ++d) {
                ...
                if (!child) continue;
                add_axis(axis_vals[d],
                         std::stoi(get_node_value(child, "order")),
            ...
            switch (axis_str[0]) {
                case 'x': this->set_axis(LegendreAxis::x); break;
                case 'y': this->set_axis(LegendreAxis::y); break;
                case 'z': this->set_axis(LegendreAxis::z); break;
                default:
    ...
    {
        if (axes_.empty()) axes_.push_back({axis, 0, -1.0, 1.0});
        else               axes_[0].axis = axis;
    }
    ...
    void SpatialLegendreFilter::set_minmax(double min, double max)
    {
        ...
        if (axes_.empty()) axes_.push_back({LegendreAxis::x, 0, min, max});
        else { axes_[0].min = min; axes_[0].max = max; }
    }
            ...
            LegendreAxis axis, int order, double min, double max)
    {
        ...
        for (const auto& ad : axes_)
            if (ad.axis == axis)
                throw std::invalid_argument {
                        fmt::format("Axis '{}' has already been added to this filter.",
                                    axis_name(axis))};
            ...
            const Particle& p, TallyEstimator /*estimator*/, FilterMatch& match) const
    {
        ...
        for (std::size_t d = 0; d < axes_.size(); ++d) {
            ...
            if (coord < ad.min || coord > ad.max) return;
    ...
    {
        int total = 1;
        for (const auto& ad : axes_) total *= ad.order + 1;
        n_bins_ = total;
    }
...
// C-API — all original functions unchanged; add_axis variant added at end.
    ...
    std::pair<int, SpatialLegendreFilter*> check_sptl_legendre_filter(int32_t index)
    {
        ...
        auto* filt = dynamic_cast<SpatialLegendreFilter*>(
                model::tally_filters[index].get());
        if (!filt) {
    ...
    extern "C" int openmc_spatial_legendre_filter_get_order(int32_t index, int* order)
    { auto [e,f]=check_sptl_legendre_filter(index); if(e) return e; *order=f->order(); return 0; }

    extern "C" int openmc_spatial_legendre_filter_get_params(
            int32_t index, int* axis, double* min, double* max)
    { auto [e,f]=check_sptl_legendre_filter(index); if(e) return e;
        *axis=static_cast<int>(f->axis()); *min=f->min(); *max=f->max(); return 0; }

    extern "C" int openmc_spatial_legendre_filter_set_order(int32_t index, int order)
    { auto [e,f]=check_sptl_legendre_filter(index); if(e) return e; f->set_order(order); return 0; }

    extern "C" int openmc_spatial_legendre_filter_set_params(
            int32_t index, const int* axis, const double* min, const double* max)
    { auto [e,f]=check_sptl_legendre_filter(index); if(e) return e;
        if(axis)     f->set_axis(static_cast<LegendreAxis>(*axis));
        if(min&&max) f->set_minmax(*min,*max);
        return 0; }
